...
MatrixXd _polyCoeff;
VectorXd _polyTime;
Vector3d _startPos  = Vector3d(0,0,1);
...
{   
    ...
    waypoints.row(0) = _startPos;               //the start position is set to 0,0,1
...
void trajGeneration(Eigen::MatrixXd path)
{
    ...
    std::cout<<"_polyTime.size() =" << _polyTime.size()<<std::endl;
    std::cout<<"_polyCoeff.size() = "<< _polyCoeff.size()<<std::endl;
    std::cout<<"_ployCoeff.cols() = "<< _polyCoeff.cols()<<"\t"<< "_polyCoeff.rows() = " \
        << _polyCoeff.rows() <<std::endl;
    std::cout<< "_polyCoeff = "<< _polyCoeff << std::endl;
    // return 8*3, which means x-y-z direction
